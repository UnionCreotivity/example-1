function pixi_fog() {
  ...
  var renderer = new PIXI2.autoDetectRenderer(width, height);//Chooses either WebGL if supported or falls back to Canvas rendering
  document.body.appendChild(renderer.view);//Add the render view object into the page

  var stage = new PIXI2.Container({
    width: width,
    height: height,
    transparent: true,
  ...
  var smokeShader = new PIXI2.AbstractFilter('', shaderCode, uniforms);

  var bg = PIXI2.Sprite.fromImage("http://www.goodboydigital.com/pixijs/pixi_v3_github-pad.png");
  bg.width = width;
  bg.height = height;
  bg.filters = [smokeShader]
  stage.addChild(bg);

  var logo = PIXI2.Sprite.fromImage("http://www.goodboydigital.com/pixijs/pixi_v3_github-pad.png");
  logo.x = width;
  logo.y = height;
  logo.blendMode = PIXI2.BLEND_MODES.ADD;
  stage.addChild(logo)
